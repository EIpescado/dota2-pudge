...
public class ChineseNumberUtils {
    ...
    private static BigDecimal textToBigDecimal(String oneUnitText) {
        ...
        for (int i = 0; i < oneUnitText.length(); i++) {
            ...
            if (CN_NUMBER_MAP.containsKey(iChar)) {
                ...
                //当前基数 是否大于 之前的基数
                boolean nowGreaterThenLast = lastUnitNumber != null && NumberUtil.isGreaterOrEqual(nowUnitNumber, lastUnitNumber);
    ...
    public static String chineseToNumber(String text) {
        ...
        for (int i = 0, length = text.length(); i < length; i++) {
            ...
            if (i == 0) {
                previousOneIsValueOrUnit = currentIsValueOrUnit;
            }
            //值 或者 单位
            if (currentIsValueOrUnit) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(chineseToNumber("万吃饭百五好贵啊"));
